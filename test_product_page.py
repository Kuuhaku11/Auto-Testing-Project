...
class TestUserAddToBasketFromProductPage():
    ...
    def setup(self, browser):
        password = 394765627648
        self.page = ProductPage(browser, link)
        self.page.open()
        self.page.go_to_login_page()
        login_page = LoginPage(browser, browser.current_url)
        login_page.register_new_user('aasdjhglfhgdfffggfdgfnjha@gmail.py', password)
        login_page.should_be_authorized_user()
        yield  # Отдельно добавил функционал удаления профиля
        self.page.go_to_profile_page()      
        profile_page = ProfilePage(browser, browser.current_url)
        profile_page.delete_profile(password)
        
    @pytest.mark.need_review
    def test_user_can_add_product_to_basket(self, browser):
        page = ProductPage(browser, link + '?promo=offer0')
        page.open()
...
def test_guest_can_add_product_to_basket(browser, num):
    page = ProductPage(browser, link + f'?promo=offer{num}')
    page.open()
    page.should_be_add_to_basket_btn()
    page.add_to_basket()
    page.solve_quiz_and_get_code()
